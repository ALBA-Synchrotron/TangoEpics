...
class TangoEpics (PyTango.Device_4Impl):
    ...
    def always_executed_hook(self):
        ...
        #----- PROTECTED REGION ID(TangoEpics.always_executed_hook) ENABLED START -----#
    ...
    def read_attr_hardware(self, data):
        ...
        #----- PROTECTED REGION ID(TangoEpics.read_attr_hardware) ENABLED START -----#
        epics.ca.use_initial_context()
        #read all PVs
        for attr_idx in data:
            attr_name = self.get_device_attr().get_attr_by_ind(attr_idx).get_name()
            ch_id = self.pv[attr_name][0]
            #we don't wait until we get the value, that will be done only once for all
    ...
    def getEpicsName(self, argin):
        ...
        try:
            ...
            argout = argout.replace('%s:' % self.Host,'') #remove the host part
        except KeyError: #attribute doesn't exist: simply return empty string
            pass
        except Exception, e:
            msg = 'Unexpected error:\n%s' % str(e)
            self.error_stream(msg)
            PyTango.Except.throw_exception('Get Epics name', msg, 'getEpicsName')
        ...
        try:
            ...
            for pv_entry in self.ProcessVariables:
                ...
                if len(pv_attr) == 1:
                    ...
                    msg = 'Invalid entry ProcessVariables device property: %s' % pv_entry
                    self.error_stream(msg)
                ...
                #create epics channel to PV (to increase performance do not wait for a connection)
                ch_id = epics.ca.create_channel(pv_full_name, connect=False, auto_cb=False)
            ...
            for attr_name, data in pvs.items():
                ...
                if tango_type == None:
                    msg = 'Unknown type %s for PV %s' % (str(data[1]),self.pv[attr_name][1])
                    self.error_stream(msg)
                ...
                if read_access and write_access:
                    ...
                    msg = 'PV %s has neither read nor write permissions' % pv_full_name
                    self.error_stream(msg)
                    self._set_state(PyTango.DevState.FAULT,msg)
                    continue
                ...
                dimension = data[4]
                if dimension > 1:
                    attr = PyTango.SpectrumAttr(attr_name,tango_type,access_type,dimension)
                else:
                    attr = PyTango.Attr(attr_name,tango_type,access_type)
                properties = PyTango.UserDefaultAttrProp()
                properties.set_description(self.pv[attr_name][1]) #set description field with PV name
    ...
    def read_attr(self, attr):
        ...
        try:
            epics.ca.use_initial_context()
            ch_id = self.pv[attr_name][0]
            #reading of PVs was requested on read_attr_hardware: now only retrieve the value
            value = epics.ca.get_complete(ch_id) 
        except Exception, e:
            msg = 'Error reading attribute %s' % attr_name
            self.error_stream('%s: %s' % (msg, str(e)))
            PyTango.Except.throw_exception('Read error',msg,'%s.read_%s()' % (self.__class__.__name__, attr_name))
        else:
    ...
    def write_attr(self, attr):
        ...
        try:
            ...
            self.error_stream('%s: %s' % (msg, str(e)))
            PyTango.Except.throw_exception('Write error',msg,'%s.write_%s()' % (self.__class__.__name__, attr_name))
    ...
    def epics2tango_name(self,pv_name):
        ...
        while len(pv_name) > 0:
            key_found = False
            #this works because keys are single chars (otherwise reg exp should be used)
    ...
    def epics2tango_type(self,epics_type):
        """
        According to http://cars9.uchicago.edu/software/python/pyepics3/ca.html#dbrtype-table:
        string        0     string
        int           1     integer
        short         1     integer
        float         2     double
...
class TangoEpicsClass(PyTango.DeviceClass):
    ...
    #----- PROTECTED REGION ID(TangoEpics.global_class_variables) ENABLED START -----#
    ...
    cmd_list = {
        ...
        'getTangoName':
            ...
            [PyTango.DevString, "Tango attribute name for the Epics process variable name"]],
        'getEpicsName':
